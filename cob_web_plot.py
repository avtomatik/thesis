...
for i in range(1, 2*len(source_frame)):
    calcul_list.append(source_frame.iloc[i//2, 1])
calcul_frame  =  pd.Data_frame(calcul_list,  columns = ["calc"])
result_frame  =  pd.concat([calcul_frame,  calcul_frame.shift(-1)],  axis = 1, 
                         sort = False).dropna()
xlin  =  np.linspace(source_frame.iloc[:, 1].min(), 
                   source_frame.iloc[:, 1].max(),  100)
ylin  =  xlin
plt.figure()
plt.plot(result_frame.iloc[:, 0], 
         result_frame.iloc[:, 1],  label = "Series",  lw = 0.5)
