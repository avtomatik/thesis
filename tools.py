...
def fetch_census(file_name: str, series_id: str) -> pd.DataFrame:
    # =============================================================================
...
def plot_cobb_douglas(data_frame: pd.DataFrame) -> pd.DataFrame:
    ...
    data_frame['lab_cap_int'] = log(data_frame['lab_cap_int'])
    Y = log(Y)
    # =============================================================================
    # Original: k=0.25
    # =============================================================================
    k, b = np.polyfit(data_frame['lab_cap_int'], Y, 1)
    b = np.exp(b)
...
def plot_cobb_douglas(data_frame):
    ...
    data_frame['lab_cap_int'] = log(
        data_frame['lab_cap_int'].astype('float64'))
...
def get_dataset_version_c():
    ...
    capital_frame = pd.concat(
        [get_cobb_douglas_extension_capital(), get_cobb_douglas_deflator()],
...
def procedure(data_frame):
    ...
    data_frame['lab_cap_int'] = signal.medfilt(data_frame['lab_cap_int'])
    data_frame['lab_product'] = signal.medfilt(data_frame['lab_product'])
    data_frame['lab_cap_int'] = np.log(data_frame['lab_cap_int'])
    data_frame['lab_product'] = np.log(data_frame['lab_product'])
    # =============================================================================
    # Original: k=0.25
...
def plot_a(source_frame):
    ...
    plt.xlabel('Period')
    plt.ylabel('Index')
    source_frame.iloc[:, 0] = source_frame.iloc[:, 0].shift(-1).add(source_frame.iloc[:, 0]).div(2)
...
def plot_cobb_douglas(data_frame):
    ...
    fig, axs = plt.subplots(4, 1)
    axs[0].plot(data_frame.iloc[:, 0], label='Fixed Capital')
    axs[0].plot(data_frame.iloc[:, 1], label='Labor Force')
    axs[0].plot(data_frame.iloc[:, 2], label='Physical Product')
    axs[0].set_xlabel('Period')
    axs[0].set_ylabel('Indexes')
    axs[0].set_title(FIGURES['fig_a'].format(data_frame.index[0],
                                             data_frame.index[-1],
                                             data_frame.index[0]))
    axs[0].legend()
    axs[0].grid(True)
    axs[1].plot(data_frame.iloc[:, 2], label='Actual Product')
    axs[1].plot(
        PP, label='Computed Product, $P\' = %fL^{%f}C^{%f}$' % (b, 1-k, k))
    axs[1].set_xlabel('Period')
    axs[1].set_ylabel('Production')
    axs[1].set_title(FIGURES['fig_b'].format(data_frame.index[0],
                                             data_frame.index[-1],
                                             data_frame.index[0]))
    axs[1].legend()
    axs[1].grid(True)
    axs[2].plot(data_frame.iloc[:, 2].sub(PR), label='Deviations of $P$')
    axs[2].plot(PP.sub(PPR), '--', label='Deviations of $P\'$')
    axs[2].set_xlabel('Period')
    axs[2].set_ylabel('Percentage Deviation')
    axs[2].set_title(FIGURES['fig_c'])
    axs[2].legend()
    axs[2].grid(True)
    axs[3].plot(PP.div(data_frame.iloc[:, 2]).sub(1))
    axs[3].set_xlabel('Period')
    axs[3].set_ylabel('Percentage Deviation')
    axs[3].set_title(FIGURES['fig_d'].format(data_frame.index[0],
                                             data_frame.index[-1]))
    axs[3].grid(True)
    plt.tight_layout()
...
def test_douglas(control, series_ids):
    ...
    if control == 'CDT2S4':
        # Total Fixed Capital in 1880 dollars (4)
        control_frame = fetch_classic('dataset_usa_cobb-douglas.zip', 'CDT2S4')
    elif control == 'J0014':
...
def options():
    ...
    [print(fetch_classic(FILE_NAME, series_id)) for series_id in SERIES_IDS]
...
def get_dataset_capital_combined_archived():
    '''Nominal Investment Series: A006RC1, 1929--1969'''
    ARCHIVE_NAME = 'dataset_usa_bea-release-2013-01-31-SectionAll_xls_1929_1969.zip'
    sub_frame_a = fetch_usa_bea(
        ARCHIVE_NAME, 'Section1ALL_Hist.xls', '10105 Ann', 'A006RC1')
    ...
    semi_frame_g = get_dataset_usa_bea_labor()
