...
from core.backend import (lookup_uscb_desc, read_get_desc, read_uscb_get_desc,
                          stockpile)
...
from core.transform import (transform_cobb_douglas, transform_fourier_discrete,
                            transform_model_capital, transform_rebase)


def plot_investment_manufacturing(df: pd.DataFrame) -> None:
    """
    ================== =================================
    df.index           Period
    ...
    plt.plot(df.iloc[:, -4:-2], label=LABEL)
    plt.xlabel("Period")
    plt.ylabel("Index")
...
def plot_investment(df: pd.DataFrame) -> None:
    """
...
def plot_manufacturing_money(df: pd.DataFrame) -> None:
    """
    ...
    plt.figure()
    plt.plot(df.iloc[:, range(2, 5)], label=LABEL)
...
def plot_d(df: pd.DataFrame, year_base: np.int64) -> None:
    """
    ...
    plt.figure(1)
    plt.semilogy(
        df.iloc[:, -2], label="Real Gross Private Domestic Investment $GPDI$"
    )
    plt.semilogy(
        df.iloc[:, -1],
        color="red",
    ...
    plt.title(
        "Real Indexes, {}=100, {}$-${}".format(*df.index[[year_base, 0, -1]])
    )
    plt.xlabel("Period")
    ...
    plt.plot(df.iloc[:, 4])
    plt.title(
        "Real Gross Domestic Product $GDP$, {}=100, {}$-${}".format(
...
def plot_e(df: pd.DataFrame) -> None:
    ...
    """
    LABEL = ["$P(I)$", "Investment to Production"]

    plt.figure()
    plt.semilogy(df.iloc[:, 0], df.iloc[:, 1])
    plt.semilogy(df.iloc[:, 0], df.iloc[:, 5])
    plt.title(
        "Investment to Production Ratio, {}$-${}".format(*df.index[[0, -1]])
    )
...
def plot_uscb_manufacturing(df: pd.DataFrame, year_base: int) -> None:
    LABEL = [
    ...
    plt.figure()
    plt.plot(df.iloc[:, [0, 2]], label=LABEL)
...
def plot_uscb_cap(df: pd.DataFrame) -> None:
    ...
    plt.title(
        "Census Manufacturing Fixed Assets, {}$-${}".format(*df.index[[0, -1]])
    )
    plt.xlabel("Period")
    plt.ylabel("Millions of Dollars")
    plt.grid()
...
def plot_uscb_cap_deflator(df: pd.DataFrame) -> None:
...
def plot_uscb_metals(df: pd.DataFrame, years_base: tuple[int]) -> None:
    _DESCS_RAW = (
...
def plot_uscb_commodities(
    df: pd.DataFrame, series_ids: dict[str, str]
) -> None:
...
def plot_uscb_immigration(df: pd.DataFrame) -> None:
    plt.figure()
    plt.plot(df)
...
def plot_uscb_unemployment_hours_worked(df: pd.DataFrame) -> None:
    plt.figure(1)
    plt.plot(df.iloc[:, 1])
    ...
    plt.title(
        "All Manufacturing, Average Full-Time Weekly Hours, 1890-1899=100"
    )
    plt.xlabel("Period")
    plt.ylabel("Percentage")
    plt.grid()
...
def plot_uscb_employment_conflicts(df: pd.DataFrame) -> None:
...
def plot_uscb_gnp(df: pd.DataFrame) -> None:
    LABEL = [
...
def plot_uscb_farm_lands(df: pd.DataFrame) -> None:
    plt.figure()
    df.plot()
...
def plot_uscb_trade(df: pd.DataFrame) -> None:
    LABEL = [
        "Exports, U1",
        "Imports, U8",
        "Net Exports, U15",
...
def plot_uscb_trade_gold_silver(df: pd.DataFrame) -> None:
    LABEL = [
        "Exports, U187",
        "Imports, U188",
        "Net Exports, U189",
...
def plot_uscb_trade_by_countries(df: pd.DataFrame) -> None:
    assert (
        df.shape[1] == 58
    ), "Works on pd.DataFrame Produced with combine_uscb_trade_by_countries()"
    _LABELS = (
        "America-Canada",
        "America-Cuba",
        "America-Mexico",
...
def plot_uscb_money_stock(df: pd.DataFrame) -> None:
    YEAR_BASE = 1915
    LABEL = [
...
def plot_approx_linear(df: pd.DataFrame, year_base, params) -> None:
    """
...
def plot_approx_linear_log(
    df: pd.DataFrame, year_base: int, params: np.ndarray
) -> None:
    """
...
def plot_lab_cap_inty_lab_prty_closure(df: pd.DataFrame) -> None:
    """
    Plotting
    ...
    plot_lab_cap_inty_lab_prty(
        *simple_linear_regression(df.iloc[:, -2:]), "Original"
    )
    plot_lab_cap_inty_lab_prty(
...
def plot_lab_cap_inty(df: pd.DataFrame) -> None:
    """
    Plotting
    ...
    plt.figure()
    plt.plot(df_o.iloc[:, 0], linewidth=3, label="Labor Capital Intensity")
    plt.plot(df_o.iloc[:, 1], label=f"Rolling Mean, {1+_k}")
    plt.plot(df_o.iloc[:, 2], label=f"Kolmogorov--Zurbenko Filter, {1+_k}")
    plt.plot(
        df_o.iloc[:, 3],
...
def plot_lab_prty(df: pd.DataFrame) -> None:
    """
    Plotting
    ...
    plt.figure()
    plt.plot(df_o.iloc[:, 0], linewidth=3, label="Labor Productivity")
    plt.plot(df_o.iloc[:, 1], label=f"Rolling Mean, {_k}")
    plt.plot(df_o.iloc[:, 2], label=f"Kolmogorov--Zurbenko Filter, {_k}")
    plt.plot(
        df_o.iloc[:, 3],
        label=f"Single Exponential Smoothing, Alpha={0.25:,.2f}",
    )
...
def plot_model_capital(df: pd.DataFrame, year_base: int) -> None:
    """
    ================== =================================
    df.index           Period
    df.iloc[:, 0]      Investment
    ...
    label = (
        "$\\lambda = {1:,.4f}\\ {0:,.4f}\\times t$".format(*params_t)
        if params_t[0] < 0
        else "$\\lambda = {1:,.4f} + {0:,.4f} \\times t$".format(*params_t)
    )
    ...
    label = (
        "$S = {1:,.4f}\\ {0:,.4f}\\times t$".format(*params_i)
        if params_i[0] < 0
        else "$S = {1:,.4f} + {0:,.4f} \\times t$".format(*params_i)
    )
    plt.plot(df.iloc[:, -5], label=label)
    plt.grid()
    plt.legend()
    plt.figure(3)
    plt.title("$\\alpha$ for the US, {}$-${}".format(*df.index[[0, -2]]))
    plt.xlabel("Period")
    plt.ylabel("Index")
...
def plot_capital_purchases(df: pd.DataFrame) -> None:
    assert (
        df.shape[1] == 27
    ), "Works on pd.DataFrame Produced with 'combine_usa_capital_purchases()"
    LABEL = [
    ...
    plt.figure()
    plt.semilogy(df.loc[:, (df.columns[0], *df.columns[-3:])], label=LABEL)
...
def plot_uscb_complex(df: pd.DataFrame) -> None:
    # =========================================================================
...
def plot_cobb_douglas(
    df: pd.DataFrame, alpha: float, scale: float, labels: dict
) -> None:
    """
    ...
    plt.xlabel("Period")
    plt.ylabel("Indexes")
    plt.title(labels["fg_a"].format(*df.index[[0, -1]], labels["year_base"]))
    plt.grid()
    plt.legend()
    plt.figure(2)
    plt.semilogy(
    ...
    plt.title(labels["fg_b"].format(*df.index[[0, -1]], labels["year_base"]))
    plt.grid()
    plt.legend()
    plt.figure(3)
    plt.plot(
    ...
    plt.xlabel("Period")
    plt.ylabel("Percentage Deviation")
    plt.title(labels["fg_d"].format(*df.index[[0, -1]]))
    plt.grid()
    ...
    plt.plot(
        lc,
        lab_productivity(lc, alpha, scale),
        label="$\\frac{3}{4}\\frac{P}{L}$",
    )
    plt.plot(
        lc,
        cap_productivity(lc, alpha, scale),
        label="$\\frac{1}{4}\\frac{P}{C}$",
    )
    plt.xlabel("$\\frac{L}{C}$")
    plt.ylabel("Indexes")
    plt.title(labels["fg_e"])
    plt.grid()
    plt.legend()
    plt.show()
...
def plot_cobb_douglas_alt(
    df: pd.DataFrame, params: tuple[tuple[float]], labels: dict
) -> None:
    """
    Cobb--Douglas Algorithm as per C.W. Cobb, P.H. Douglas. A Theory of Production, 1928;
    """
...
def plot_cobb_douglas_complex(df: pd.DataFrame) -> None:
    """
    ...
    plot_cobb_douglas(_df, _params, get_figure_labels(YEAR_BASE))
...
def plot_douglas(
    ...
    for _, (series_ids, title, ylabel, label) in enumerate(
        ...
        plt.figure(_)
        plt.plot(stockpile(series_ids), label=label)
        plt.title(title)
        plt.xlabel("Period")
        plt.ylabel(ylabel)
        plt.grid()
...
def plot_elasticity(df: pd.DataFrame, plot_title: tuple[str]) -> None:
    """
...
def plot_ewm(df: pd.DataFrame, step: float = 0.1) -> None:
    ...
    _alpha = 0.25
    # =========================================================================
    # pd.DataFrame for Exponentially Smoothed Series
    # =========================================================================
    _smooth = df.copy()
    while True:
        if _alpha >= 1:
        ...
        _smooth.columns = [
            *_smooth.columns[:-1],
            f"{_smooth.columns[0]}_{_alpha:,.2f}",
        ]
        _alpha += step
    # =========================================================================
    # pd.DataFrame for Deltas of Exponentially Smoothed Series
    # =========================================================================
    _deltas = pd.concat(
        [
    ...
    _labels = [
        "Original Series",
        *[
            "Smoothing: $\\alpha={:,.2f}$".format(float(column.split("_")[-1]))
            for column in _smooth.columns[1:]
        ],
    ]
    plt.figure(1)
...
def plot_fourier_discrete(df: pd.DataFrame, precision: int = 10) -> None:
    ...
    df[f"{df.columns[0]}_fourier_{0}"] = (
        np.cos(df.iloc[:, 1].mul(0))
        .mul(df_fourier.loc[0, "cos"])
        .add(np.sin(df.iloc[:, 1].mul(0)).mul(df_fourier.loc[0, "sin"]))
    )
    for _ in range(1, precision):
        df[f"{df.columns[0]}_fourier_{_}"] = (
            df.iloc[:, -1]
            .add(np.cos(df.iloc[:, 1].mul(_)).mul(df_fourier.loc[_, "cos"]))
            .add(np.sin(df.iloc[:, 1].mul(_)).mul(df_fourier.loc[_, "sin"]))
        )
        plt.plot(
            df.iloc[:, 2].add(df.iloc[:, -1]),
...
def plot_growth_elasticity(df: pd.DataFrame) -> None:
    ...
    # Series, Growth Rate
    # =========================================================================
    _df[f"{df.columns[1]}_growth_rate"] = (
        df.iloc[:, [1]].diff(2).div(df.iloc[:, [1]].rolling(2).sum().shift(1))
    )
    # =========================================================================
    # Series, Elasticity
    # =========================================================================
    _df[f"{df.columns[1]}_elasticity"] = (
        df.iloc[:, [1]]
        .rolling(2)
        .sum()
        .shift(-1)
        .mul(2)
        .div(df.iloc[:, [1]].rolling(4).sum().shift(-1))
        .sub(1)
    )
...
def plot_increment(
    ...
    fig.tight_layout()
    if savefig:
        fig.savefig(BASE_DIR / file_name, format="pdf", dpi=900)
    else:
        plt.show()


def plot_filter_kol_zur(df: pd.DataFrame) -> None:
    """
    ...
    plt.ylabel("Measure")
    plt.scatter(
        df_o.iloc[:, [0]].index, df_o.iloc[:, [0]], label="Original Series"
    )
    plt.plot(
        df_o.iloc[:, 1:],
        label=[
            "$KZF(\\lambda = {})$".format(int(_.split("_")[-1], 16))
    ...
    plt.plot(
        ...
        label=[
            "$\\delta KZF(\\lambda = {})$".format(int(_.split("_")[-1], 16))
...
def plot_usa_kurenkov(*args) -> None:
    ...
    *args : TYPE
    ...
    axes[0].plot(
        next(data_frame_gen), label=["Kurenkov Data", "BEA Data", "FRB Data"]
    )
    ...
    axes[0].legend()
    axes[0].grid()
    axes[1].plot(next(data_frame_gen), label=["Kurenkov Data", "BEA Data"])
    ...
    # Revised Capital
    # =========================================================================
    axes[2].plot(next(data_frame_gen), label=["Kurenkov Data", "BEA Data"])
    axes[2].set_title(f"Capital, {1950}=100")
    axes[2].set_xlabel("Period")
    axes[2].set_ylabel("Percentage")
    axes[2].legend()
    axes[2].grid()
    axes[3].plot(next(data_frame_gen), label=["Kurenkov Data", "FRB Data"])
...
def plot_lab_prod_polynomial(df: pd.DataFrame) -> None:
    ...
    polyfit_quadratic = np.polyfit(
        df.iloc[:, -2].astype(float), df.iloc[:, -1].astype(float), deg=2
    )
    polyfit_cubic = np.polyfit(
        df.iloc[:, -2].astype(float), df.iloc[:, -1].astype(float), deg=3
    )
    polyfit_quartic = np.polyfit(
        df.iloc[:, -2].astype(float), df.iloc[:, -1].astype(float), deg=4
    )
    # =========================================================================
    # pd.DataFrame for Approximation Results
    # =========================================================================
    _df = pd.DataFrame()
    ...
    plt.figure(1)
    plt.scatter(
        df.iloc[:, [-1]].index, df.iloc[:, [-1]], label="Labor Productivity"
    )
    plt.plot(
        _df.iloc[:, 0],
...
def plot_pearson_r_test(df: pd.DataFrame) -> None:
    ...
    for _ in range(1 + df.shape[0] // 2):
        ...
        _pearson.loc[_] = [
            ...
            / stats.pearsonr(df.iloc[:, 0][df_l.notna()], df_l.dropna(axis=0))[
                0
            ]
        ]
    # =========================================================================
...
def plot_filter_rolling_mean(df: pd.DataFrame) -> None:
    """
    Rolling Mean Filter
    ...
    plt.figure(1)
    plt.title(f"Rolling Mean {df_o.index[0]}$-${df_o.index[-1]}")
    plt.xlabel("Period")
    plt.ylabel("Percentage")
    plt.scatter(
        df_o.iloc[:, [0]].index, df_o.iloc[:, [0]], label="Original Series"
    ...
    plt.plot(
        df_o.iloc[:, 1:],
        label=[
            "$\\hat Y_{{m = {}}}$".format(int(_.split("_")[-1], 16))
            for _ in df_o.columns[1:]
        ],
    )
    plt.plot(
    ...
    plt.legend()
    plt.figure(2)
    plt.title(f"Rolling Mean Residuals {df_o.index[0]}$-${df_o.index[-1]}")
    plt.xlabel("Period")
    ...
    plt.plot(
        df_residuals_o.iloc[:, 1:],
        label=[
            "$\\delta(\\hat Y_{{m = {}}})$".format(int(_.split("_")[-1], 16))
            for _ in df_residuals_o.columns[1:]
        ],
    )
    plt.plot(
...
def plot_lab_cap_inty_lab_prty(
    df: pd.DataFrame, params: tuple[float], option: str
) -> None:
    """
    ================== =================================
    df.index           Period
    ...
    plt.figure(1)
    plt.plot(df.iloc[:, 0], df.iloc[:, 1], label=option)
    ...
    plt.title(
        MAP[option][2]["title"].format(*params[::-1], *df.index[[0, -1]])
    )
