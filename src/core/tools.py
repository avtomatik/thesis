...
def calculate_capital(
    df: pd.DataFrame, p_i: tuple[float], p_t: tuple[float], ratio: float
) -> pd.Series:
    """
...
def calculate_plot_uspline(df: pd.DataFrame):
    ...
    plt.plot(df.iloc[:, 0], spl(df.iloc[:, 0]), "g", lw=3, label="Spline")
    plt.title(
    ...
    print(spl.integral(1.0, 1.75))
    print(spl.roots())
    print(spl.set_smoothing_factor(0.25))
    plt.show()


def filter_kol_zur(df: pd.DataFrame, k: int = None) -> tuple[pd.DataFrame]:
    """
    ...
    # =========================================================================
    # pd.DataFrame for Kolmogorov--Zurbenko Filter Results: Odd
    # =========================================================================
    df_o = pd.concat(
        [
            # =================================================================
    ...
    # =========================================================================
    # pd.DataFrame for Kolmogorov--Zurbenko Filter Results: Even
    # =========================================================================
    df_e = pd.concat(
        [
            # =================================================================
    ...
    # =========================================================================
    # pd.DataFrame for Kolmogorov--Zurbenko Filter Residuals: Odd
    # =========================================================================
    ...
    # =========================================================================
    # pd.DataFrame for Kolmogorov--Zurbenko Filter Residuals: Even
    # =========================================================================
...
def lash_up_spline_ea(
    df: pd.DataFrame, n_spans: int, knots: tuple[int]
    ...
    for j in range(n_spans):
        _params_a.append(
            (
                (df.iloc[knots[1 + j], 0] - df.iloc[knots[0], 0])
                * np.log(df.iloc[knots[j], 1])
                - (df.iloc[knots[j], 0] - df.iloc[knots[0], 0])
                * np.log(df.iloc[knots[1 + j], 1])
            )
        ...
        if j == 0:
            ...
            _params_k.append(
                ...
                + np.log(df.iloc[knots[1 + j], 1])
                / (df.iloc[knots[1 + j], 0] - df.iloc[knots[j], 0])
                - (df.iloc[knots[1 + j], 0] - df.iloc[knots[j - 1], 0])
                * np.log(df.iloc[knots[j], 1])
                / (
                    (df.iloc[knots[1 + j], 0] - df.iloc[knots[j], 0])
        ...
        if 1 + j == n_spans:
            for i in range(knots[j], 1 + knots[1 + j]):
                _splined.append(
                    np.exp(
                        _params_a[j]
                        + _params_k[j] * (df.iloc[i, 0] - df.iloc[0, 0])
                    )
                )
        else:
...
def lash_up_spline_eb(
        ...
        if 1 + j == n_spans:
            for i in range(knots[j], 1 + knots[1 + j]):
                _splined.append(
                    df.iloc[knots[j], 1]
                    * np.exp(
                        _params_k[j] * (df.iloc[i, 0] - df.iloc[knots[j], 0])
                    )
                )
        else:
            for i in range(knots[j], knots[1 + j]):
    ...
    return (
        pd.concat(
            [df, pd.DataFrame(_splined, columns=("Spline"))], axis=1, sort=True
        ),
        tuple(_params_k),
    )


def _lash_up_spline_error_metrics(df: pd.DataFrame) -> None:
    """Error Metrics Function"""
    print(
        "Criterion, C: {:.6f}".format(
            df.iloc[:, 2].div(df.iloc[:, 1]).sub(1).abs().mean()
        )
    )


def lash_up_spline_la(
    df: pd.DataFrame, n_spans: int, knots: tuple[int]
) -> tuple[pd.DataFrame, tuple[float]]:
    """
...
def lash_up_spline_lls(
    ...
    for _ in range(n_spans):
        if 1 + _ == n_spans:
            ...
            _s_xy = (
                df.iloc[knots[_] : knots[1 + _], 0]
                .mul(df.iloc[knots[_] : knots[1 + _], 1])
                .sum()
            )
            if _ == 0:
                _params_a.append(
    ...
    for _ in range(n_spans):
        if _ == 0:
            ...
            _params_k.append(
                _params_k[_ - 1]
                + (_params_a[_] - _params_a[1 + _]) * df.iloc[knots[_], 0]
            )

        if 1 + _ == n_spans:
            for _i in range(knots[_], 1 + knots[1 + _]):
                _splined.append(
                    _params_k[_] + _params_a[1 + _] * df.iloc[_i, 0]
                )
        else:
...
def run_lash_up_spline(df: pd.DataFrame, kernel: callable) -> None:
    """
    ...
    if _go_no_go.lower() == "y":
        ...
        _correction_factors = [
            float(
                input(
                    f"Correction Factor of Knot {1 + _:02d} out of {len(_knots):02d}: "
                )
            )
...
def price_direct(df: pd.DataFrame, year_base: int) -> pd.DataFrame:
    """
...
def price_inverse(df: pd.DataFrame) -> pd.DataFrame:
    """
...
def price_inverse_double(df: pd.DataFrame) -> pd.DataFrame:
    """
...
def filter_rolling_mean(
    ...
    df.reset_index(level=0, inplace=True)
    # =========================================================================
    # pd.DataFrame for Rolling Mean Filter Results: Odd
    # =========================================================================
    df_o = pd.concat(
        [
            # =================================================================
    ...
    # =========================================================================
    # pd.DataFrame for Rolling Mean Filter Results: Even
    # =========================================================================
    df_e = pd.concat(
        [
            # =================================================================
    ...
    # =========================================================================
    # pd.DataFrame for Rolling Mean Filter Residuals: Odd
    # =========================================================================
    df_residuals_o = pd.concat(
        [
            # =================================================================
    ...
    # =========================================================================
    # pd.DataFrame for Rolling Mean Filter Residuals: Even
    # =========================================================================
    df_residuals_e = pd.concat(
        [
            # =================================================================
...
def simple_linear_regression(
    ...
    params, _ess, *_ = np.polyfit(
        df.iloc[:, 0], df.iloc[:, 1], deg=1, full=True
    )
    # =========================================================================
    # Approximation
    # =========================================================================
